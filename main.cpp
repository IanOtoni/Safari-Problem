...
int main() {
    ...
    node_to_cage_map.push_back(-1); ////Mapeamento de cada nó (vértice) para o índice da sua jaula, para saber se a jaula de um nó já foi visitada

    for(int i=0; i<num_cages; i++) { //para cada jaula
        for(Point &vertex : cages[i]){ //para cada vertice da jaula
            nodes.push_back(vertex); //adiciona em nodes
            node_to_cage_map.push_back(i); //mapeia i no na caixa i
        }
    }
    node_to_cage_map.push_back(-1); //ultimo no nao tem jaula
    ...
    for(int iter=0; iter<NUM_ITERATIONS; iter++){
        ...
        for(int ant=0; ant<NUM_ANTS; ant++){
            ...
            vector<bool> visited_cages(num_cages, false); //rastreia jaulas visitadas
            int current_node        = 0;
